...
class ShopModel extends CommonModel{
	...
	public function buildShopQrcode($shop_id,$size = '0'){
		...
		$size = $size ? $size : '15';
        $url = $config['site']['host'].''.U('wap/shop/detail', array('shop_id'=>$shop_id,'t' => NOW_TIME,'sign' =>md5($shop_id.C('AUTH_KEY').NOW_TIME)));
	...
	public function shop_audit($shop_id){
		...
		if($detail = $this->find($shop_id)){
			if(empty($detail['express_price'])){
				...
				return false;
			}elseif($detail['express_price'] < 300 ){
				$this->error = '配送费必须大于3元';
				return false;
