   ...
   //商城万能打印接口
	...
	public function get_order_settlement_price_intro($detail){
		...
		if($estimated_price > 0){
			if($shop['is_goods_pei'] == 1){
				$express_price = isset($shop['express_price']) ? (int)$shop['express_price'] : 300;//商家自己配置的默认运费
				if($detail['express_price'] < $express_price){
					$settlement_price = $estimated_price - $express_price;
					$express_price = $express_price;
					$intro .='状态：【已开通配送状态，用户支付运费小于商家默认配送费】---';   
					$intro .='结算金额：结算价'.round($detail['need_pay']/100,2).'-商家默认配送费'.round($express_price/100,2).'元'.'-佣金'.round($commission/100,2).'元】---';   
					...
					$intro .='结算金额：结算价'.round($detail['need_pay']/100,2).'-用户支付运费'.round($detail['express_price']/100,2).'元'.'-佣金'.round($commission/100,2).'元】---';   
					$intro .='当前佣金费率：【'.round($shop['commission']/100,2).'%】';   
				}
