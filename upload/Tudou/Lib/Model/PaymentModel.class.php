        ...
        if (!empty($logs) && !$logs['is_paid']) {
            ...
            if (D('Paymentlogs')->save($data)) { //总之 先更新 然后再处理逻辑  这里保障并发是安全的
                ...
                if ($logs['type'] == 'appoint') {
					...
					//D('Marketorder')->market_delivery_order($logs['order_id'],0);//菜市场配送接口
					...
					//D('Storeorder')->store_delivery_order($logs['order_id'],0);//便利店配送接口
					...
					return true;
                }elseif ($logs['type'] == 'community') {//小区回调
