...
class ShopAction extends CommonAction{
	...
	public function _initialize(){
        ...
        $this->assign('grades',$grades = D('Shopgrade')->where(array('closed'=>0))->select());//土豆二开增加商家等级
    }
    ...
    private function editCheck($shop_id){
		...
		if ($data['express_price'] < 100) {
            $this->tuError('配送费必须大于1元');
        }
		$data['commission'] = (int) ($data['commission']*100);
		if($data['commission'] < 0) {
            $this->tuError('结算佣金不能小于0，可以等于0');
        }
