...
class BreaksAction extends CommonAction {
    ...
    public function envelope($envelope_id = 0,$order_id = 0,$orderType = 0){
		...
		$money = (int)((($Breaksorder['need_pay']*100)*($Envelope['ratio']*100))/100000);
		...
		if($money > 0){
			$intro = '买单订单ID【'.$order_id.'】领取红包【'.round($money/100,2).'】';
			...
			if(M('EnvelopeLogs')->add($arr)){
				...
				$this->ajaxReturn(array('code'=>'1','msg'=>$intro,'url'=>U('breaks/detail',array('order_id'=>$order_id))));        
			}else{
