...
class BookingAction extends CommonAction {
    ...
    public function roomcreate(){
         ...
         if(IS_POST){
             ...
             if($obj->add($data)){
                 $this->tuSuccess('恭喜你操作成功',U('booking/roomcreate'));
             }
             $this->tuError('操作失败');
         }else{             
             $this->assign('types',$obj->getType());
        ...
        $obj = D('Bookingroom');
         if(IS_POST){
             ...
             if(false !== $obj->save($data)){
                 $this->tuSuccess('恭喜你操作成功',U('booking/roomedit',array('room_id'=>$room_id)));
             }
             $this->tuError('操作失败');
         }else{ 
    ...
    public function menuedit($menu_id = 0) {
        if ($menu_id = (int) $menu_id) {
            ...
            if (!$detail = $obj->find($menu_id)) {
                $this->tuError('请选择要编辑的菜品设置');
            }
            if ($detail['shop_id'] != $this->shop_id) {
                $this->tuError('请不要操作其他商家的菜品设置');
            }
            if ($this->isPost()) {
