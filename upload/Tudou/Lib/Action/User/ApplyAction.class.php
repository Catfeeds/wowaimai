...
class ApplyAction extends CommonAction{
    ...
    public function index(){
		...
		if($shop_apply_prrice > 0){
			if($this->member['money'] < $shop_apply_prrice){
				$this->error('余额不足，不能入驻商家！请先充值'.round($shop_apply_prrice/100,2).'元后操作', U('money/index'));
			}
		}
        if($this->isPost()){
            ...
            if ($shop_id = $obj->add($data)) {
				...
				D('Users')->addMoney($this->uid,-$shop_apply_prrice,'商家名称【'.$data['shop_name'].'】入驻扣除费用');//商家入驻扣除费用
				D('Shop')->buildShopQrcode($shop_id,15);//生成商家二维码
                $this->tuMsg('恭喜您申请成功', U('member/index'));
            }
            $this->tuMsg('申请失败');
        } else {
    ...
    private function createCheck(){
        ...
        if (!isImage($data['photo'])) {
            $this->tuMsg('商家形象图格式不正确');
        }
		$data['logo'] = htmlspecialchars($data['logo']);
        if (empty($data['logo'])) {
            $this->tuMsg('请上传商家LOGO');
        }
        if (!isImage($data['logo'])) {
            $this->tuMsg('商家LOGO格式不正确');
        }
